...
ChiefSlider.prototype._setActiveClass = function () {
  ...
  for (var i = 0, length = $itemList.length; i < length; i++) {
    ...
    if (i == +$itemList.length - 1 && activeItems.indexOf(index) > -1 && ifLastSlideActive == 0) {
      $itemList[0].classList.add('last_slide_active');
    } else {
      $itemList[0].classList.remove('last_slide_active');
    }
  }
  ifLastSlideActive = 0;
};
...
ChiefSlider.prototype._updateIndicators = function () {
  ...
  if (currentSlider > $itemList.length - 1) {
    currentSlider = 0;
  } else if (currentSlider < 0) {
    currentSlider = $itemList.length - 1;
  }
  $indicatorList[currentSlider].classList.add(CLASS_INDICATOR_ACTIVE);
};
